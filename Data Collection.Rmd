...
inputted_gapminder %>% 
   na.omit() %>% 
   filter(year==2005) %>% 
   ggplot(aes(x=contraceptive_use_percent_of_women_ages_15_49, y=life_expectancy_ratio)) +
   geom_point()

inputted_gapminder %>% 
   na.omit() %>% 
   filter(year==2005) %>% 
   ggplot(aes(x=mean_years_in_school_women_percent_men_25_to_34_years, y=life_expectancy_ratio)) +
   geom_point()
...
confint(lm.numerical, "mean_years_in_school_women_percent_men_25_to_34_years")
...
confint(lm.numerical,"contraceptive_use_percent_of_women_ages_15_49")
confint(lm.numerical, "mean_years_in_school_women_percent_men_25_to_34_years")
summary(lm.numerical)
```
...
```{r, fig.width=10, fig.height=6}
female_model_map_large<-inputted_gapminder %>%
   filter(year==2005) %>% 
   na.omit() %>%  
   augment(lm.numerical_female, data=.) %>% 
   full_join(countries, by=c("country"="NAME"))

female_model_map_large %>% 
   ggplot() +
   scale_fill_viridis("Female life expectancy predictions")+
   geom_sf(aes(fill = .fitted)) +
   ggtitle("Model Predictions Female Life Expectancy 2005")

diff_model_map_large<-inputted_gapminder %>%
   filter(year==2005) %>% 
   na.omit() %>%  
   augment(lm.numerical_diff, data=.) %>% 
   full_join(countries, by=c("country"="NAME"))

diff_model_map_large %>% 
   ggplot() +
   scale_fill_viridis("Life expectancy difference predictions")+
   geom_sf(aes(fill = .fitted)) +
   ggtitle("Model Predictions Female Life Expectancy 2005")


ratio_model_map_large<-inputted_gapminder %>%
   filter(year==2005) %>% 
   na.omit() %>%  
   augment(lm.numerical_ratio, data=.) %>% 
   full_join(countries, by=c("country"="NAME"))

ratio_model_map_large %>% 
   ggplot() +
   scale_fill_viridis("Life expectancy ratio predictions")+
   geom_sf(aes(fill = .fitted)) +
   ggtitle("Model Predictions Female Life Expectancy 2005")
